...
export class Table {
  ...
  getPlayerSpotMap(players: PlayerTableInfo[]): {[k: string]: number} | void {
  ...
  getOccupiedActiveSpotCount(): number | void {
  ...
  getPlayerBySpotId(spotId: number): Player | void {
    // return this.players.find(({ spotIds }) => spotIds.includes(spotId))
  }

  getPlayerByHandle(handle: string): Player | void {
  ...
  dealerHasBlackjack(): boolean | void {
    // return this.dealerHand.hasBlackjack();
  }

  getDealerUpCard(): string | void {
